...
const postToParent = (level: string, text: string, extra: unknown) => {
  try {
    if (isBackend() || !window.parent || window.parent === window) {
      const consoleWithDynamicLevel = console as Console & Record<string, (...args: unknown[]) => void>;
      const writer =
        typeof consoleWithDynamicLevel[level] === 'function'
          ? consoleWithDynamicLevel[level]
          : console.log;
      writer(text, extra);
      return;
    }
